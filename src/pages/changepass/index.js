...
export const Changepass = () => {
  ...
  const handleLoginSubmit = async () => {
    if (password !== confirmPassword) {
      ...
      // console.log(token)
      await axios.patch("http://localhost:8081/user/change",{username: username, newPassword: password,}).then(response=> {
        console.log("2");
      ...
      if (role === "ARTIST") {
        ...
        navigate(`/artistdash/${userid}`); // Redirect to artist dashboard with userid
      } else if (role === "MANAGER") {
        console.log('Login successful');
        navigate(`/managerdash/${userid}`); // Redirect to manager dashboard with userid
      } else if (role === "ADMIN") {
        console.log('Login successful');
